...
{
public:
    TextQuery(ifstream&is):file(new StrVec)//初始化空的向量容器
    {   
        string text;
        while(getline(is,text)){
            ...
            string word;
            while(line>>word){
                auto &lines=wm[word];//lines为一个指针,注意这个&
...
public:
QueryResult(string s,shared_ptr<set<line_no>>p,StrVec*f):
sought(s),lines(p),file(f){};
const set<line_no>::iterator begin(){return lines->begin();}//set容器的迭代器是默认const
const set<line_no>::iterator end(){return lines->end();}//这里用引用出了返回局部变量紧告
StrVec*& getfile(){return file;}
private:
    string sought;
