...
int start_sq_polling_ops(struct io_uring *ring) {
  ...
  fds[1] = open(FILE_NAME2, O_RDWR | O_TRUNC | O_CREAT, 0644);
  if (fds[1] < 0) {
    perror("open");
    return 1;
  }
  ...
  io_uring_prep_write(sqe, 1, buff1, str1_sz, 0);
  sqe->flags |= IOSQE_FIXED_FILE;
  ...
  io_uring_prep_write(sqe, 1, buff2, str2_sz, str1_sz);
  sqe->flags |= IOSQE_FIXED_FILE;
